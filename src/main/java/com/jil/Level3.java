...
public class Level3 implements Screen {
    ...
    public void deleteSaveGame() {
        FileHandle saveFile = Gdx.files.local("savegame3.json");
    ...
    public void saveGameState() {
        ...
        Gdx.files.local("savegame3.json").writeString(jsonString, false);

        Gdx.app.log("Save", "Game state saved successfully.");
    }

    public GameState loadGameState() {
        if (!Gdx.files.local("savegame3.json").exists()) {
            Gdx.app.log("Load", "No saved game found.");
            return null;
        }

        Json json = new Json();
        String jsonString = Gdx.files.local("savegame3.json").readString();
...
//        camera.update();
        ...
        backgroundTexture = new Texture("BG4.jpg");
...
//            {13.5f + 3f, 2.75f, 0.25f, 1f, 1f, "Toons_Stone_Block.png"},
    ...
    public void render(float delta) {
        ...
        if(currentBirdIndex == 5){
            backgroundMusic.stop();
            ResultScreen3 r = new ResultScreen3((Main)game,score);
            game.setScreen(r);
        }
